...
class Generator(BaseModel):
    ...
    def fill_logger(self):
        if not self.logger:
            self.logger = logger.get_logger("EntitiesInCoordenatesPolar",
...
class EntitiesInCoordenatesPolar(Generator):
    ...
    def create_ontology(self):
        ...
        self.logger.info("Creating EntitiesInCoordenatesPolar",
                         answer=self.answer,
                         e=e,
                         c=c)
        ...
        if self.answer == 0:
            i = self.states_qty - 1
            condition = (lambda x:
                         (e, c) not in x and (e, self.uncertainty) not in x)
            states[i] = self.initialize_state(
                i, condition)  # NewEntityInLocationState
            for j in list(reversed(range(i))):
                any_condition = lambda x: True
        ...
        if self.answer == 2:
            if random.choice([0, 1]):
                ...
                for j in range(i + 1, len(states)):
                    if (j == i + 1
                        ):  # Here we place the entity in the self.uncertainty.
...
class EntitiesInCoordenatesWho(Generator):
    ...
    def create_ontology(self):
        ...
        if self.answer == 1:
            i = self.states_qty - 1
            # only_a_in_l = a \in l && all(A[1:], lambda x : x \not \in l)
            ...
            raise NotImplementedError("Answer 0 not implemented yet")

        elif self.answer == 2:
            raise NotImplementedError("Answer 2 not implemented yet")
        else:

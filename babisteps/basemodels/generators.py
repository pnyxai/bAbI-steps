...
class SimpleTracker(BaseModel):
    ...
    def create_fol(self):
        ...
        for s in self.states[0:-1]:
            sentences.extend(describe_transitions(s))
        self.fol = sentences
...
class ComplexTracking(BaseModel):
    ...
    def fill_logger(self):
        if not self.logger:
            self.logger = logger.get_logger("ComplexTracking",
                                            level=self.verbosity)
        return self
    ...
    def create_fol(self):
        ...
        def describe_states(state) -> list[list]:
            ...
            for (loc, a) in actors_in_locations_nothing.data:
                state_sentences.append(
                    In(entity=self.dim1_idx_to_obj[a],
                        ...
                        state_sentences.append(
                            In(entity=self.dim1_idx_to_obj[a],
                               ...
                               shape_str=(self.shape_str[0],
                                          self.shape_str[1])))
                state_sentences.append(
                    In(entity=self.dim2_idx_to_obj[o],
        ...
        def describe_transitions(state: State) -> list[list]:
            ...
            delta_mappings = {
                (1, 0): (self.dim1_idx_to_obj, self.dim0_idx_to_obj,
                         ("Location", "Actor"), self.uncertainty[0]),
                (2, 1): (self.dim2_idx_to_obj, self.dim1_idx_to_obj,
                         ("Actor", "Object"), self.uncertainty[1]),
            }
        ...
        sentences = []
        for t, dim_str in zip(self.model.as_tuple, self.shape_str):
            sentences.extend(enumerate_model(t[:-1], dim_str))
        sentences.extend(describe_states(self.states[0]))
        for s in self.states[0:-1]:
            sentences.extend(describe_transitions(s))
        self.fol = sentences
